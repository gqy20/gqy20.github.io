...
git hooks 正是补这一环的天然位置。把测试挂进 `pre-commit` 或 `pre-push`,TDD 的「绿」就从「我记得跑」升级成「不跑过就提交不了」——**人治变法治**。这也是第 6 节那条「别在 hook 里做重活」最该落地的地方:
...
> 💡 **TDD 和 hook 是同构的**:TDD 说「先写测试定义契约,再写实现满足契约」,hook 说「先定义检查点,再执行操作」——两者本质都是**契约前置 + 节点强制**。所以把 TDD 的测试挂进 git hook 不是两个东西的拼接,而是同一套思想的重叠:测试用例就是「代码该满足的契约」,hook 就是「在版本控制节点上强制执行契约的机制」。
...
搭这套系统的过程中,我犯过一个想当然的错误,后来翻源码才想明白——值得单独记一笔,因为它最容易把人带偏。
