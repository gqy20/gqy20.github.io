...
本文不会只教你「怎么配一个 hook」,而是带你从 git hooks 的底层机制出发,看懂这套思想是如何**迁移**到 AI Coding 时代的,以及我自己用一套 hook 系统给 Claude Code 装上了「APM(应用性能监控)」。文中所有配置和脚本都来自我的开源项目。
...
| `prepare-commit-msg` | 准备好默认提交信息后、编辑器启动前 | ✅ 阻断(不吃 `--no-verify`) | 自动生成 commit message(如分支名) |
...
> 💡 这正是 GitHub Actions、GitLab CI 的**思想来源**——一次 push 触发一次流水线,在范式层面就是一个更强大、更标准的 `post-receive`(实现上它们并非真的挂在 git 服务端跑 hook,而是 push 事件经 webhook → 独立 runner 重新 checkout 后执行;但「推送即触发流水线」的精神同源)。

### 2.4 痛点与演进:`.git/hooks` 是个「孤岛」

git hooks 有个固有的设计缺陷:**`.git/hooks` 不进版本控制**。
...
| [**simple-git-hooks**](https://github.com/toplenboren/simple-git-hooks) | Node | 极简,零依赖 |
...
### 2.5 git hooks × TDD:让「测试通过」从自觉变成强制

TDD(测试驱动开发)的红-绿-重构循环里,「绿」——测试通过——是每次代码变更的安全底线。但 TDD 有个老毛病:它依赖**人的自觉**。赶进度时 `git commit` 一回车就过去了,「记得跑测试」很快沦为一句空话。

git hooks 正是补这一环的天然位置。把测试挂进 `pre-commit` 或 `pre-push`,TDD 的「绿」就从「我记得跑」升级成「不跑过就提交不了」——**人治变法治**。

- `pre-commit` 跑**全量**测试 = 反模式,每次提交等两分钟,大家很快开始无脑 `--no-verify`,护栏形同虚设;
- 正确分层:`pre-commit` 只跑**快**的(受影响的单测、lint、类型检查,秒级返回),全量测试交给 `pre-push` 或 CI。

> 💡 **TDD 和 hook 是同构的**:TDD 说「先写测试定义契约,再写实现满足契约」,hook 说「先定义检查点,再执行操作」——两者本质都是**契约前置 + 节点强制**。测试用例就是「代码该满足的契约」,hook 就是「在版本控制节点上强制执行契约的机制」。

这套不是我纸上谈兵。我的 [`bitcat`](https://github.com/gqy20/bitcat) 是 Rust 项目,用 `cargo-husky` 把 fmt、clippy 和测试(400+ 个)挂进 git hook,推送前测试必须绿;而 [`cc_plugins`](https://github.com/gqy20/cc_plugins) 里的 `err-catcher.py` 更进一步——它挂在 `PostToolUse`,用 `guess_phase` 根据命令推断当前在 TDD 的哪一环(`pytest`→test、`git commit`→git、`ruff`→lint),并按错误模式把失败归类成 `TEST-FAIL`/`TIMEOUT`/`PERMISSION`,让 agent 时代的「测试失败」也能被结构化地感知。

### 2.6 真实案例:我的 `quick-template` 模板里的 git hooks
...
面对这样的执行体,「在固定时刻注入人类意志」就成了刚需。而 hook,正是这个「注入点」。
...
> 💡 这张表是**概念上的对照**(事前拦截 / 事后观察 / 收尾审查的语义平行),便于从 git hooks 迁移理解 Claude Code hooks,而**不是实现层面的 1:1 映射**——两者是完全独立的系统,事件来源与契约细节都不同。

### 3.3 护栏(guardrail)理论:为什么 AI Coding 时代必须 hooks

按作用我把 hook 能干的事分成三类——它也是后面所有实战的纲领:
...
- **matcher**:用正则匹配工具名(注意**大小写敏感**,`Write` 不会匹配 `write`)。其中**匹配「工具名」的 `matcher` 字段只有 `PreToolUse` 和 `PostToolUse` 这两个事件可用**;此外 `SessionStart`、`PreCompact` 走的是各自的按 `source`/`trigger` 过滤(见 4.2 表),其余事件不使用 matcher。`Write|Edit` 表示匹配写文件类工具,`mcp__.*` 能匹配所有 MCP 工具,`*`(或留空)匹配全部。
...
> ⚠️ **`Stop` 要防死循环**:用 `Stop` 阻止 agent 停下时,输入里会带一个 `stop_hook_active` 字段——为 `true` 表示「这次继续正是上一轮 Stop hook 触发的」。务必检查这个标志(或读 transcript),否则 Claude 会陷入「被阻止停止 → 继续工作 → 又触发 Stop → 又被阻止」的死循环。
...
> ⚠️ **逐事件有差异**:`exit 2` 的 stderr **并非所有事件都喂回 Claude**。`PreToolUse`/`PostToolUse`/`Stop`/`SubagentStop` 会把 stderr 交给 Claude;而 `UserPromptSubmit`(擦除并拒绝 prompt)、`Notification`、`PreCompact`、`SessionStart`、`SessionEnd` 的 `exit 2` 只给用户看、不进上下文。判断「这条报错要不要被 AI 看到」时,务必对照官方的逐事件表。
...
看一个我真实在用的 hook——`detect-task-complete.sh`,它挂在 `PostToolUse` 的 `TaskUpdate` matcher 上,作用是「当某个任务被标记为 completed 时,自动扫描它产出的技能、标记为可升级」:
...
这套「订阅事件 → 转发 → 可视化」的链路,不是纸上谈兵——我开源的 [`bitcat`](https://github.com/gqy20/bitcat) 桌宠里就有一条完整、可验证的实现,叫 **Agent Watch**:

- **订阅**:在用户的 `~/.claude/settings.json` 里装一条只读 hook,覆盖全部 9 个官方事件,不审批、不改 prompt、纯观察;
- **转发**:PowerShell 脚本读 stdin 的 JSON,包成带版本号的 envelope(`schema: bitcat.agent-hook.v1`),TCP 发到本地 `127.0.0.1:5342`,失败只记日志、`exit 0` 绝不阻塞 Claude;
- **可视化**:桌宠侧的 monitor 在浮窗里实时展示会话状态、运行中命令、hook 事件,还能跨机器远程看管 LAN/Tailscale 上的 Claude Code/Codex。

上面 AI_PAD 的泛化配置是我的个人采集器,bitcat 则是这条链路公开、完整、带远程能力的参考实现(详见 [Agent Watch Hooks 文档](https://github.com/gqy20/bitcat/blob/main/docs/guide/agent-watch-hooks.md))。

### 5.3 一个想当然的错误:hook 的真正开放面在「下游」

搭这套系统的过程中,我犯过一个想当然的错误,后来翻源码才想明白。

我一度以为:既然 AI_PAD 里出现了 `PostToolBatch`、`PostToolUseFailure`、`PermissionRequest`、`PermissionDenied` 这样的「事件名」,而官方文档里又没有它们,那一定是 Claude Code 的 hook 是一个「开放契约」,允许我**自定义事件、扩展事件总线**。

但事实是——**这是错的**。

Claude Code **只会触发那 9 个官方事件**(官方 [hooks reference](https://docs.claude.com/en/docs/claude-code/hooks) 的 "Hook Events" 章节只列了这 9 个,别无分号)。你在 `settings.json` 里写一个叫 `PostToolUseFailure` 的事件名,它**永远不会被触发**。那这些「事件」是从哪来的?答案很朴素:**是下游脚本自己派生出来的**。

- `PostToolUseFailure` 这类「失败事件」,机制上和我 [`cc_plugins`](https://github.com/gqy20/cc_plugins) 里的 [`err-catcher.py`](https://github.com/gqy20/cc_plugins) 一模一样:后者老老实实挂在官方的 `PostToolUse` 上,但拿到 Bash 的 `tool_response` 后,用正则判断「这行输出到底是不是失败了」——`FAILED`/`AssertionError` 归类成 `TEST-FAIL`、`timeout` 归类成 `TIMEOUT`、`Permission denied` 归类成 `PERMISSION`。所谓「失败事件」,是脚本**算**出来的,不是 Claude Code **报**出来的。
- `PostToolBatch`(一批工具调用结束)同理,是采集器把连续的 `PostToolUse` 聚合后贴的标签;`PermissionRequest`/`PermissionDenied` 则是从 `PreToolUse` 的 permissionDecision 里推断出来的。
- 更典型的是 [`bitcat`](https://github.com/gqy20/bitcat) 的 **Agent Watch**:Claude Code / Codex 的 hook 把会话事件包装成一个**统一的 envelope**,转发给桌宠本地的 TCP monitor,桌宠侧只读地展示「最近会话、当前状态、运行中命令、hook 事件、离开提醒」。bitcat 看到的「事件」是它自己定义的 envelope,但**源头仍然是官方那几个事件**。

所以,真正「开放」的从来不是事件总线,而是**下游的数据加工层**。

> 💡 **修正之前的二分**:我曾据此得出「git hooks 是闭口系统、Claude Code hooks 是开口系统」——这个说法**站不住**。两者的事件枚举其实**都是封闭的**:git 把事件写死在二进制里,Claude Code 也只给那 9 个。真正的区别在于**输入的结构化程度**——git hooks 给你的多是原始的文件路径、stdin 里的 ref 列表;而 Claude Code hooks 给你的是**带完整参数的结构化 JSON**(`tool_input.command`、`tool_input.file_path`、`tool_response.success`……)。正因为输入足够结构化,你才能在下游精确地派生新分类、跨系统转发、做 APM——这是 git hooks 难以企及的表达力,但它来自「输入丰富」,不来自「事件可扩展」。
...
- Claude Code 端:官方默认每个 hook **60 秒超时**,且 hook **会阻塞**主循环——虽然同一事件下多个匹配的 hook 是**并行**跑、重复命令会自动去重的,但主循环仍要等它们全部完成。你要是在 `PostToolUse` 里同步调一个慢 API,agent 就会卡在那里干等。
...
不管 git hooks 还是 Claude Code hooks,写好一个 hook 的原则是相通的:
...
当 AI agent 越来越多地替我们写代码、跑运维、做决策,我们最需要的不是更强的模型,而是一套**可靠的、可审计的、可扩展的接口**,让我们能定义「什么能做、什么不能做、做完要知道」。hook,就是这套接口。git 在 2005 年用 `.git/hooks` 给了我们第一版;Claude Code 在 2025 年用 `settings.json` 的 hooks 给了我们针对 AI agent 的第二版;而我在 2026 年用 AI_PAD 证明了——**事件枚举固然封闭,但 hook 拿到的结构化输入足够开放,任何人都能在下游造出属于自己的第三版、第四版系统。**

下次当你给项目加一个 hook,记得:你写的是**你和代码执行者(人或 AI)之间的一份契约**。
...
- [`cc_plugins`](https://github.com/gqy20/cc_plugins) —— Claude Code 插件与 hook 脚本集合(`err-catcher.py`、`notify.sh` 等)
- [`bitcat`](https://github.com/gqy20/bitcat) —— 第 2.5 节 cargo-husky 守测试、第 5.3 节 Agent Watch 消费 hook 事件的桌宠
