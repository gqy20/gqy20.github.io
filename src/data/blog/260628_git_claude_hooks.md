...
| 章节 | 一句话主旨 | 阅读时长 |
|------|-----------|---------|
| **零、背景与动机** | 两道拦截背后的同一套思想 | 3 分钟 |
| **一、Git Hooks 的本质** | 把逻辑挂进版本控制的生命周期 | 5 分钟 |
| **二、范式迁移** | 当代码由 AI 写,hooks 该挂在哪里 | 3 分钟 |
| **三、Claude Code Hooks 详解** | 事件、输入/输出契约、配置 | 5 分钟 |
| **四、实战:Agent 的 APM** | 我用 hook 全量上报 + 可视化 Claude 行为 | 4 分钟 |
| **五、两层防线协同** | git hooks × claude hooks 如何配合 | 2 分钟 |
| **六、设计哲学** | 写好一个 hook 的五条军规 | 1 分钟 |

---

## 一、Git Hooks 的本质:把逻辑挂进版本控制的生命周期
...
### 5️⃣ 真实案例:我的 `quick-template` 模板里的 git hooks
...
## 二、范式迁移:当代码不再由人写,hooks 该挂在哪里?

### 1️⃣ git hooks 抓不住 AI
...
### 2️⃣ Claude Code hooks:把钩子挂到「代理行为生命周期」
...
### 3️⃣ 护栏(guardrail)理论:为什么 AI Coding 时代必须 hooks
...
## 三、Claude Code Hooks 详解:事件、契约与配置

### 1️⃣ 配置在哪:三层 settings + 插件
...
## 四、实战:我用 hooks 给 Claude Code 装了一套「Agent 的 APM」

这一章讲我怎么用 hook,把 Claude Code 从一个黑盒,变成一个**全程可观测的系统**。

### 1️⃣ 痛点:AI agent 是个黑盒
...
`sender.sh` 收到 stdin 里的 JSON,补上时间戳和来源标记,异步发到一个本地服务。注意是**异步上报、立即 `exit 0`**——这是可观测类 hook 的铁律:**绝不能让「记录」拖慢「执行」**(见第六章)。

### 3️⃣ 一个有趣的发现:hook 是「开放契约」
...
> 💡 **这就是「AI Agent 的 APM」**。后端有 Datadog、SkyWalking 做应用性能监控;CI 有 GitHub Actions 的运行时长统计;而 AI agent 时代,我们用 **hook + 一个采集器 + 一个看板**,给 agent 装上同样的可观测性。把这套和第一章的 git `post-receive` 通知对比一下:git 的可观测是「一次推送触发一次通知」的离散事件;Claude Code 的可观测是「全链路、全工具调用」的连续观测——**维度从「一个 commit」升级到了「整个 agent 行为」**。

---

## 五、git hooks × Claude Code hooks:两层防线如何协同
...
- [`cc-insights`](https://github.com/gqy20/cc-insights) —— 第四章「Agent 的 APM」可视化工具(Go)
